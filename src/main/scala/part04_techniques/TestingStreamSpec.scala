...
class TestingStreamSpec
    extends TestKit(ActorSystem("testing-akka-stream"))
    with WordSpecLike
  ...
  "A simple stream" should {
    ...
    "integrate with Streams TestKit Sink" in {
      ...
      val materializedTestValue: TestSubscriber.Probe[Int] =
        sourceUnderTest.runWith(testSink)
    ...
    "test flow with a test source and a test sink" in {
      ...
      publisher.sendNext(1).sendNext(2).sendNext(3).sendComplete()

      subscriber.request(3).expectNext(2, 4, 6).expectComplete()
